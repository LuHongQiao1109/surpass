...
// 在 [`interface Box<T>`]，`T` 是**泛型类型参数**（type parameter）。

// 它的意思是：先不写死 [`value`] 的具体类型，等使用 [`Box`] 时再指定。

// 例如：
// - [`Box<string>`]：`T` 就是 `string`
// - [`Box<number>`]：`T` 就是 `number`

// 所以 [`value: T`] 可以复用成多种类型，同时保持类型安全。
